...
# Make a file that will send instructions to Clustal with appropriate name of input file
	...
	if ($pauser eq "clustalw") {
		...
		$muscleoutfile=$out_file . '.aa_ali.fasta';	
		print STDERR "/programs/mafft/bin/mafft --auto $muscleinfile > $muscleoutfile\n";
		open(LOG, ">$out_file.$pauser.log") || die "Cannot create file outfile.pauser.log: $out_file.$pauser.log\n";
		my $pid = open(PH, "/programs/mafft/bin/mafft --auto $muscleinfile 2>&1 1>$muscleoutfile |");              # or with an open pipe
		while(<PH>) {
			print STDERR $_;
			print LOG $_;
		}
		close(LOG);
		#system "/programs/mafft/bin/mafft --auto $muscleinfile > $muscleoutfile";
	}
